...
CREATE INDEX idx_pairs_network ON pairs(network);
CREATE INDEX idx_pairs_base_token ON pairs(base_token);
...
CREATE INDEX idx_orders_trigger_price ON orders(trigger_price) WHERE order_type IN ('stop_loss', 'take_profit');
...
CREATE INDEX idx_fills_pair_id ON fills(pair_id);
CREATE INDEX idx_fills_order_id ON fills(order_id);
CREATE INDEX idx_fills_network ON fills(network);
