...
type MatchingEngine struct {
        ...
        // OnOrderExpired is called after an order is successfully marked expired.
        // The handler wires this to broadcast a WS order_update + orderbook refresh.
        OnOrderExpired func(order models.Order)
...
type MatchRequest struct {
        ...
        IsPostOnly        bool            // Post-only: reject if would fill immediately
...
func NewMatchingEngine(cfg *config.Config, db *db.DB, balanceRepo *repository.BalanceRepository, refundService *services.RefundService) *MatchingEngine {
        orderRepo := repository.NewOrderRepository(db)
        fillRepo := repository.NewFillRepository(db)
...
func (e *MatchingEngine) Start(ctx context.Context) {
        // Load order books in background so route registration is not blocked.
        // The matching engine is safe to use before loading completes — requests
        // against pairs that haven't loaded yet simply see empty order books,
        // which is correct when there are no open orders.
        go e.loadOrderBooks(ctx)
...
func (e *MatchingEngine) processMatchRequest(ctx context.Context, req *MatchRequest) {
        ...
        // Post-only guard: if the incoming order is post-only, check at match time
        // whether it would immediately fill. If yes, cancel it immediately — never execute.
        // This is the correct professional behaviour: post-only orders must rest on the book.
        if req.IsPostOnly {
                ob.mu.RLock()
                wouldFill := false
                if req.Side == models.OrderSideBuy {
                        for _, level := range ob.Asks {
                                if req.Price.GreaterThanOrEqual(level.Price) {
                                        wouldFill = true
                                        break
                                }
                        }
                        ...
                        for _, level := range ob.Bids {
                                if req.Price.LessThanOrEqual(level.Price) {
                                        wouldFill = true
                                        break
                                }
                        }
                ...
                if wouldFill {
                        ...
                        result.Error = fmt.Errorf("post-only order would immediately fill — cancelled")
                        req.ResultChan <- result
                        return
                }
                // Does not cross the book — allow it to rest as a maker order (no fills)
                result.Status = models.OrderStatusOpen
                req.ResultChan <- result
                return
        }
...
func (e *MatchingEngine) processExpiredOrders(ctx context.Context) {
        ...
        for _, o := range orders {
                ...
                // Notify subscribers (WS broadcast, orderbook refresh) via callback
                if e.OnOrderExpired != nil {
                        e.OnOrderExpired(o)
                }
