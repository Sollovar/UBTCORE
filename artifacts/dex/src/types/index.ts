...
export interface Pair {
  ...
  // Exchange prices (from actual fills/orderbook on our backend)
  price: number;
  priceUSD?: number;
  ...
  // GeckoTerminal reference prices (always set by price-worker, independent of fills)
  // These are the market reference prices from GeckoTerminal API
  geckoPrice?: number;
  geckoPriceUSD?: number;
  geckoPriceChange24h?: number;
  geckoHigh24h?: number;
  geckoLow24h?: number;
  geckoLiquidity?: number;
  geckoLiquidityUSD?: number;
  geckoMarketCap?: number;
  geckoMarketCapUSD?: number;
