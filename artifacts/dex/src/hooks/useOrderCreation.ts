...
export function useOrderCreation() {
  ...
  const createOrderWithSignature = useCallback(async (params: OrderParams): Promise<OrderResult> => {
    ...
    try {
      // Validate wallet compatibility with the target network
      const walletChain = primaryWallet?.chain;
      const isSolanaWallet = walletChain === 'SOL' || 
                            primaryWallet?.connector?.key === 'phantom' || 
                            primaryWallet?.connector?.key === 'solflare';
      const isEVMWallet = walletChain === 'EVM' || 
                         primaryWallet?.connector?.key === 'metamask' ||
                         primaryWallet?.connector?.key === 'walletconnect' ||
                         primaryWallet?.connector?.connectedChain;

      if (params.network === 'solana' && !isSolanaWallet) {
        throw new Error('Please connect a Solana wallet (e.g., Phantom, Solflare) to trade on Solana network');
      }

      if (params.network !== 'solana' && isSolanaWallet && !isEVMWallet) {
        throw new Error(`Please connect an EVM wallet (e.g., MetaMask) to trade on ${params.network.toUpperCase()} network`);
      }
      ...
      if (params.network === 'solana') {
        ...
        if (typeof (primaryWallet as any).getWalletClient !== 'function') {
          throw new Error('EVM wallet client not available — please reconnect your wallet');
        }
      ...
      if (params.network !== 'solana') {
        if (!baseTokenAddress || !quoteTokenAddress) {
          throw new Error('Token contract addresses are not available for this pair. Please try again or select a different pair.');
        }
      }
      ...
      console.log('[useOrderCreation] Sending order to backend:', {
        network: params.network,
        pairId: params.pairId,
        side: params.side,
        orderType: orderPayload.order_type,
        price: params.price,
        amount: orderPayload.amount,
        amountIn: orderPayload.amount_in,
        maker: userAddress,
        tokenIn,
        tokenOut,
        signature: orderSignature?.substring(0, 20) + '...',
        hasDepositInfo: params.network === 'solana' && !!params.depositTxHash,
      ...
      console.log('[useOrderCreation] Calling backend API...');
      const order = await createOrder(orderPayload);
      console.log('[useOrderCreation] Backend response received:', {
        id: order.id,
        status: order.status,
      ...
      console.error('[useOrderCreation] Order creation failed:', {
        error: err.message,
        stack: err.stack,
        network: params.network,
        pairId: params.pairId,
