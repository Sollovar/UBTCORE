...
export function normalizeApiPair(p: any): Pair {
  // base_token_info / quote_token_info are the authoritative source — populated by the
  // Node.js indexer from GeckoTerminal with full metadata (description, links, image, etc.)
  // base_token / quote_token is a legacy column that may contain corrupted/escaped JSON
  // (\\\\"\\\\) for many rows — we use it only as a last-resort fallback for symbol/name/address.

  let baseTokenInfo  = parseTokenField(p.base_token_info);
  let quoteTokenInfo = parseTokenField(p.quote_token_info);

  // Discard token_info if it has no symbol — treat as absent
  if (!baseTokenInfo.symbol)  baseTokenInfo  = null as any;
  if (!quoteTokenInfo.symbol) quoteTokenInfo = null as any;

  // Parse base_token / quote_token as fallback only
  const rawBase  = parseTokenField(p.base_token);
  ...
  let baseToken: Record<string, any> = {
    ...
    logo:     (baseTokenInfo?.image_url || baseTokenInfo?.image_large || baseTokenInfo?.image_small || baseTokenInfo?.image_thumb || baseTokenInfo?.logo)
              || rawBase.logo || '',
    decimals: baseTokenInfo?.decimals   ?? rawBase.decimals ?? p.base_token_decimals ?? 18,
    website:  baseTokenInfo?.websites?.[0] || rawBase.website || '',
    links: {
    ...
    about: baseTokenInfo?.description || '',
  };

  // Ensure symbol fallback from top-level plain columns
  ...
  let quoteToken: Record<string, any> = {
    ...
    logo:     (quoteTokenInfo?.image_url || quoteTokenInfo?.image_large || quoteTokenInfo?.image_small || quoteTokenInfo?.image_thumb || quoteTokenInfo?.logo)
              || rawQuote.logo || '',
    decimals: quoteTokenInfo?.decimals   ?? rawQuote.decimals ?? p.quote_token_decimals ?? 18,
    website:  quoteTokenInfo?.websites?.[0] || rawQuote.website || '',
    links: {
    ...
    about: quoteTokenInfo?.description || '',
  };
  ...
  // This field is ONLY set when actual fills exist on our backend
  const lastTradePrice = parseFloat(p.last_trade_price) || undefined;
  const exchangePriceChange = parseFloat(p.price_change_24h ?? p.priceChange24h);
  const priceChange = (lastTradePrice !== undefined && !Number.isNaN(exchangePriceChange))
    ? exchangePriceChange
    : 0;
  ...
  // NEVER use geckoPrice here - pair.price must reflect actual trading activity
  const exchangePrice = lastTradePrice ?? 0;

  // Derive exchange USD price using the gecko quote token rate
  // quoteTokenUSDRate = geckoPriceUSD / geckoPrice
  // exchangePriceUSD = exchangePrice * quoteTokenUSDRate
  const exchangePriceUSD = exchangePrice > 0 && geckoPriceUSD && geckoPrice > 0
    ? (exchangePrice / geckoPrice) * geckoPriceUSD
    : undefined;
  ...
  return {
    ...
    baseToken,
    quoteToken,
    price: exchangePrice,  // Exchange price from fills (0 if no trades yet)
    priceUSD: exchangePriceUSD,  // Derived exchange USD price
    ...
    // GeckoTerminal reference prices — always from price-worker, separate from exchange prices
    geckoPrice: geckoPrice,
    geckoPriceUSD: geckoPriceUSD,
    geckoPriceChange24h: geckoPriceChange24h,
    geckoHigh24h: parseFloat(p.gecko_high_24h) || undefined,
    geckoLow24h: parseFloat(p.gecko_low_24h) || undefined,
    geckoLiquidity: parseFloat(p.gecko_liquidity) || undefined,
    geckoLiquidityUSD: parseFloat(p.gecko_liquidity_usd) || undefined,
    geckoMarketCap: parseFloat(p.gecko_market_cap) || undefined,
    geckoMarketCapUSD: parseFloat(p.gecko_market_cap_usd) || undefined,
    // Last actual fill price from our matching engine
